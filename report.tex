...
%  Dr. Teddy Lazebnik
%
%  Compiled with: pdfLaTeX (default Overleaf compiler)
%
%  IMAGES TO UPLOAD TO OVERLEAF ROOT:
%    full_learning_curves.png          (from full_training_results/)
%    full_convergence_detail.png       (from full_training_results/)
%    full_policy_comparison.png        (from full_training_results/)
%    hyperparameter_tuning.png         (project root)
%    reward_ablation_learning.png      (project root)
%    reward_ablation_comparison.png    (project root)
%    value_functions_comparison.png    (project root)
% ============================================================
...
Q-Learning agent achieves a \textbf{14\%} improvement in average episode reward
($-923$ vs.\ $-1077$) and reduces average waiting cars per step from $3.63$ to
$3.12$. Compared with fixed-time switching policies, the improvement reaches
\textbf{73\%}. We additionally conduct a reward function ablation study across four
...
$\gamma{=}0.95$) are optimal. Together, these results demonstrate that even lightweight
tabular RL agents can learn near-optimal adaptive signal control strategies and
consistently outperform rule-based controllers.
\end{abstract}
...
\midrule
\textbf{Trained Q-Learning} & $\mathbf{-923.1}$ & $\mathbf{3.116}$ \\
Trained DQN                 & $-933.6$          & $3.155$ \\
Trained SARSA               & $-946.1$          & $3.195$ \\
\midrule
Longest Queue First (LQF)   & $-1{,}076.9$      & $3.633$ \\
Fixed-Time (10-step)        & $-3{,}122.4$      & $10.417$ \\
Random Switch               & $-3{,}386.5$      & $11.293$ \\
Fixed-Time (5-step)         & $-3{,}507.6$      & $11.696$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{RL vs.\ LQF}: the best RL agent (Q-Learning) reduces average waiting
          cars from $3.63$ to $3.12$ --- a \textbf{14\%} improvement in queue
          management.

    \item \textbf{RL vs.\ Fixed-Time}: against the best fixed-time controller
          (10-step), the improvement is \textbf{70\%} in reward and $3.3\times$ fewer
          waiting cars per step.

    \item \textbf{Q-Learning $\geq$ DQN $\geq$ SARSA}: all three RL agents are
          statistically close ($<3\%$ spread in reward). The slight edge of Q-Learning
          and DQN over SARSA is consistent with their off-policy training (maximising
          over future actions regardless of exploration).
...
range. Q-Learning achieves the best final evaluation reward, but all three RL agents
...
\begin{enumerate}[noitemsep]
    \item \textbf{RL outperforms all baselines}, including the greedy LQF heuristic, in
          ...
          improvement over LQF is $\sim$14\% in reward and waiting-time metrics.
    ...
    \item \textbf{DQN matches tabular performance in $12\times$ fewer episodes},
          demonstrating the sample-efficiency advantage of neural function approximation
          even in small state spaces.
